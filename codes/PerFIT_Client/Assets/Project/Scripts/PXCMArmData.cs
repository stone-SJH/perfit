...
public class PXCMArmData : MonoBehaviour {
	...
	void Start () {
		...
		joints = new JointData[3];
		smoother3D = new PXCMDataSmoothing.Smoother3D[3];
		...
		for (int i=0; i<=2; i++) 
		{
			...
			smoother3D [i] = ds.Create3DWeighted (5);
		}
		smoother4D = ds.Create3DWeighted (5);
	...
	Alert CheckStatus(float x1,float y1, float z1,float x2, float y2, float z2, float x3, float y3, float z3)
	{
		old_ca = ca;
		old_ab = ab;
		...
		Debug.LogWarning(x1.ToString()+" "+y1.ToString()+" "+z1.ToString());
	...
	bool FindShoulder(int height, int width)
	{
		...
		for (int y = height - 3; y <= height - 2; y++)
		{
			...
			for (x = width - 2; x >= 0; x--)
			{
				if (!IsEdge(x, y, width)) {targetx = x;p = true; break;}
			}
		//	Debug.Log(x);
			for ( ; x >= 0; x--)
			{
		...
		for ( int y = 0; y < height - 1 ; y++ ) 
		{
			...
			{	
				...
				if (!backGroundTriangles) {
					...
					if (Mathf.Abs((float)(x1-x2)) > Armlength)
						if (Mathf.Abs((float)Mathf.Abs((float)(xx-x1))-(float)Mathf.Abs((float)(x2-xx))) > 0.4)
							continue;
					if (yy > y1 || xx > x2-10) continue;
