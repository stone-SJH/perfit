...
public class setShoudlerResult : MonoBehaviour {
	Dictionary<int,string> dic = new Dictionary<int,string>();
	string Info = "";
	string[] Infos;
	string[] name = {
		"1","2","3","平均时间","最长时间","最短时间","完成度得分","稳定性得分","波动度得分"
	};
	
	public void HoverOver(GameObject obj){
		int number = int.Parse (obj.name);
		string detail = dic [number+3].Substring(0,5);
	...
	void Start () {
		FileOperation fo = new FileOperation ("shoulder_result.txt");
